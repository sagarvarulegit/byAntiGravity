# HyperFrames Video Generator — AI Memory

Auto-loaded by agy on startup. Keep this file updated as conventions evolve for the Hyperframes video generator.

## Project
HyperFrames video generator — programmatic HTML/GSAP animated video exports. Replaces the legacy Remotion React-based generator.

## Conventions

- **Interactive Studio Selection (CRITICAL)**: Every visual element (e.g., characters, ships, lighthouses, text labels, backgrounds) MUST be rendered as an independent, absolute-positioned HTML DOM element (like `<img>`, `<div>`, `<p>`).
  - **Do NOT** group disparate graphic assets into a single monolithic `<svg>` canvas, as this treats the entire block as one unselectable box model in HyperFrames Studio.
  - If SVG paths/lines are required for drawing effects (like lines of sight or angles), they must be placed in a transparent overlay `<svg>` canvas with `pointer-events: none;` applied to it. This ensures the overlay does not block mouse clicks, allowing the user to select, drag, and freely resize the underlying `<img>` or `<div>` assets individually in the studio.
...
- **Studio Interactivity Support**: Always wrap visual scene elements (SVG shapes, images, groups, cards, badges, text lines) in `Interactive.G` or `Interactive.Div` with clear human-readable `name` attributes (e.g. `name="Lighthouse Structure"`, `name="Near Ship"`, `name="GOLDEN RULE Card"`). This allows creators to click, drag, re-align, and scale any visual element directly in Remotion Studio without asking for manual coordinate code edits.
  ...
  - **Clickable SVG Paths**: To make individual SVG lines or paths selectable without blocking the HTML elements below them, apply `pointer-events: none;` to the parent `<svg>` canvas, and apply `style="pointer-events: auto; cursor: pointer;"` directly to the `<line>` or `<path>` elements.
