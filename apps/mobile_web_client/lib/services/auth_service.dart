...
/// Local mock authentication service for testing/offline mode.
class MockAuthService implements AuthService {
  final _controller = StreamController<AuthUser?>.broadcast();
  AuthUser? _currentUser;

  MockAuthService() {
    // Start in logged-out state asynchronously so stream listeners can subscribe first.
  ...
  Future<AuthUser> signIn(String email, String password) async {
    ...
    final String displayName = name[0].toUpperCase() + name.substring(1);

    _currentUser = AuthUser(
      id: "mock-uid-12345",
      email: email.trim(),
      fullName: displayName,
      isPremium: email.toLowerCase().contains('premium'),
    );
