...
def patch_file(filepath, replacements):
    ...
    # Decode using utf-8 with replacement for any invalid bytes
    content = content_bytes.decode('utf-8', errors='replace')
    
    # Normalize newlines to \n for consistent matching
    content_normalized = content.replace("\r\n", "\n")
    
    original_len = len(content_normalized)
    patched_count = 0
    
    for target, replacement in replacements:
        # Normalize target newlines as well
        target_norm = target.replace("\r\n", "\n")
        replacement_norm = replacement.replace("\r\n", "\n")
        
        if target_norm in content_normalized:
            content_normalized = content_normalized.replace(target_norm, replacement_norm)
            patched_count += 1
        else:
            print(f"  Warning: Target content not found in file!")
            print(f"    Target search: {target_norm[:100]}...")
            
    if patched_count > 0:
        # Convert back to system newlines if on Windows (or just keep \n, PostgreSQL handles both)
        content_to_write = content_normalized.replace("\n", "\r\n")
        with open(filepath, "wb") as f:
            f.write(content_to_write.encode('utf-8', errors='replace'))
        print(f"  Successfully applied {patched_count} of {len(replacements)} patches. Length: {original_len} -> {len(content_normalized)}")
        return True
    else:
